...
export const Kanban: React.FC = () => {
  ...
  function onDragEnd(event: DragEndEvent) {
    ...
    const isActiveABucket = active.data.current?.type === "Bucket";

    if (!isActiveABucket) return;

    setBuckets((buckets) => {
      const activeIndex = buckets.findIndex((bucket) => bucket.id === activeId);
      const overIndex = buckets.findIndex((bucket) => bucket.id === overId);
      return arrayMove(buckets, activeIndex, overIndex);
    });
  }
  ...
  function onDragOver(event: DragOverEvent) {
    ...
    if (isActiveACard && isOverACard) {
      setCards((cards) => {
        ...
        cards[activeIndex].step = cards[overIndex].step;
  ...
  return (
    <div className="w-full h-full flex flex-1 mx-auto">
      <DndContext
          ...
          <SortableContext items={buckets.map((b) => b.id)}>
            {buckets.map((bucket) => (
              <KanbanBucket
                key={bucket.id}
                bucket={bucket}
                cards={cards.filter((c) => c.step === bucket.id)}
          ...
          {typeof document !== "undefined" &&
            createPortal(
              <DragOverlay>
                {activeBucket && (
                  <KanbanBucket
                    cards={cards.filter((c) => c.step === activeBucket.id)}
                    bucket={activeBucket}
                    onClickCard={handleClientClick}
