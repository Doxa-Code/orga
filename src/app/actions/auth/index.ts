...
import { JWTTokenCreatorDriver } from "@/core/infra/drivers/token-authenticate-creator";
...
export const authenticateWithCode = createServerAction()
  ...
  .handler(async ({ input }) => {
    ...
    (await cookies()).set(PAYLOAD_KEY_TOKEN, JSON.stringify(payload));
    redirect("/dashboard");
  });
...
export const getPayload = createServerAction()
  ...
  .handler(async () => {
    return JSON.parse((await cookies()).get(PAYLOAD_KEY_TOKEN)?.value || "{}");
  });

export const checkAuthenticateAction = createServerAction().handler(
  async () => {
    if (!(await cookies()).has(PAYLOAD_KEY_TOKEN)) {
