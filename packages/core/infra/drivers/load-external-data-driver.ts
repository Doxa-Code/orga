...
import type { AddressRaw } from "../../application/mappers/address-mapper";
import type { CityRaw } from "../../application/mappers/city-mapper";
import type { StateRaw } from "../../application/mappers/state-mapper";
...
export class BrasilApiLoadExternalDataDriver implements LoadExternalDataDriver {
  ...
  async listState(): Promise<StateRaw[]> {
    try {
      const response =
        await this.request.get<ResponseStateDTO[]>("/ibge/uf/v1");
      return response.data.map<StateRaw>((state) => ({
        acronym: state.sigla,
        name: state.nome,
      }));
    } catch (err) {
  ...
  async retrieveCities(acronymState: string): Promise<CityRaw[]> {
    try {
      ...
      return response.data.map<CityRaw>((state) => ({
        name: state.nome,
      }));
    } catch (err) {
      console.log(err);
  ...
  async retrieveAddressByZipCode(zipcode: string): Promise<AddressRaw | null> {
    try {
