...
const timelineDotVariants = cva(
  "col-start-2 col-end-3 bg-transparent row-start-1 row-end-1 flex size-5 items-center justify-center rounded-full border border-current",
  {
    variants: {
      status: {
        default: "[&>*]:hidden",
        ...
        error:
          "border-destructive [&>*:not(.lucide-x)]:hidden [&>.lucide-x]:text-background",
        done: "",
...
const timelineContentVariants = cva(
  "row-start-2 row-end-2 pb-8 font-light text-sm text-muted-foreground",
  {
    variants: {
      side: {
...
const timelineHeadingVariants = cva(
  "row-start-1 row-end-1 line-clamp-1 max-w-full truncate",
  {
    variants: {
      ...
      variant: {
        primary: "text-sm font-normal text-primary",
